...
.id-note-toggle {
  ...
  padding: 4px 0; display: block; margin-top: 4px;
}
`;
...
function buildHeader(data: InternalDocData): string {
  ...
  <div class="id-header">
    <div class="id-header-top">
      ...
      <span class="id-status-badge ${statusClass}">${esc(data.project.status)}</span>
    </div>
...
function buildGoal(data: InternalDocData): string {
  ...
  <div class="id-goal">
    <div class="id-goal-summary">${esc(data.goal.summary)}</div>
...
function buildTechStack(data: InternalDocData): string {
  ...
  const rowsHTML = data.tech.map((t: TechEntry) => `
    <tr>
      ...
      <td class="id-tech-notes">${esc(t.notes)}</td>
    </tr>`).join('');
